...
// Official Dispatch Import Pipeline Service (v2)
...
function headers(token: string) {
  if (!token) throw new Error('Sevkiyat yüklemesi için yetkili v2 oturumu gerekli.');
...
async function call<T>(path: string, token: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(body.code || body.error || 'DISPATCH_REQUEST_FAILED');
  return body as T;
}
...
export const dispatchImportService = Object.freeze({
  parse: (batchId: string, token: string, rows: any[] = []) => 
    call(`/imports/dispatches/${batchId}/parse`, token, { method: 'POST', body: JSON.stringify({ rows }) }),
  validate: (batchId: string, token: string) => call(`/imports/dispatches/${batchId}/validate`, token, { method: 'POST' }),
  publish: (batchId: string, token: string, input: { expectedValidationRunId: string; idempotencyKey: string }) => 
    call(`/imports/dispatches/${batchId}/publish`, token, { method: 'POST', body: JSON.stringify(input) }),

  async uploadAndPreview(file: File, token: string, parsedRows: any[] = [], sourceKind: 'SEVKIYAT_SIPARISLER' | 'SEVKIYAT_BELGELER' = 'SEVKIYAT_SIPARISLER') {
    const declaredSha256 = await sha256(file);
    const idempotencyKey = `dispatch-init-${crypto.randomUUID()}`;
    ...
    if (!upload.ok) throw new Error('DISPATCH_STORAGE_UPLOAD_FAILED');
    ...
    await this.publish(initiated.batchId, token, {
      ...
      idempotencyKey: `dispatch-pub-${crypto.randomUUID()}`
    });
