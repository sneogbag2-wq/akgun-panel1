...
// Official Purchase Import Pipeline Service (v2)
...
function headers(token: string) {
  if (!token) throw new Error('Satın alma yüklemesi için yetkili v2 oturumu gerekli.');
...
async function call<T>(path: string, token: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(body.code || body.error || 'PURCHASE_REQUEST_FAILED');
  return body as T;
}
...
export const purchaseImportService = Object.freeze({
  parse: (batchId: string, token: string, rows: any[] = []) => 
    call(`/imports/purchases/${batchId}/parse`, token, { method: 'POST', body: JSON.stringify({ rows }) }),
  validate: (batchId: string, token: string) => call(`/imports/purchases/${batchId}/validate`, token, { method: 'POST' }),
  publish: (batchId: string, token: string, input: { expectedValidationRunId: string; idempotencyKey: string }) => 
    call(`/imports/purchases/${batchId}/publish`, token, { method: 'POST', body: JSON.stringify(input) }),

  async uploadAndPreview(file: File, token: string, parsedRows: any[] = []) {
    const declaredSha256 = await sha256(file);
    const idempotencyKey = `purchase-init-${crypto.randomUUID()}`;

    const initiated = await call<{ batchId: string; upload: { signedUrl: string } }>('/imports/initiate', token, {
      method: 'POST',
      body: JSON.stringify({
        sourceKind: 'SATIN_ALMA',
    ...
    if (!upload.ok) throw new Error('PURCHASE_STORAGE_UPLOAD_FAILED');
    ...
    await this.publish(initiated.batchId, token, {
      ...
      idempotencyKey: `purchase-pub-${crypto.randomUUID()}`
    });
