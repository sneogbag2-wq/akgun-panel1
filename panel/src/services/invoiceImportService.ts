// panel/src/services/invoiceImportService.ts
// Official Sales Invoice Import Pipeline Service (v2)
...
function headers(token: string) {
  if (!token) throw new Error('Fatura yüklemesi için yetkili v2 oturumu gerekli.');
...
async function call<T>(path: string, token: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(body.code || body.error || 'SALES_INVOICE_REQUEST_FAILED');
  return body as T;
}
...
export const invoiceImportService = Object.freeze({
  parse: (batchId: string, token: string, rows: any[] = []) => 
    call(`/imports/sales/${batchId}/parse`, token, { method: 'POST', body: JSON.stringify({ rows }) }),
  validate: (batchId: string, token: string) => call(`/imports/sales/${batchId}/validate`, token, { method: 'POST' }),
  publish: (batchId: string, token: string, input: { expectedValidationRunId: string; idempotencyKey: string }) => 
    call(`/imports/sales/${batchId}/publish`, token, { method: 'POST', body: JSON.stringify(input) }),

  async uploadAndPreview(file: File, token: string, parsedRows: any[] = []) {
    const declaredSha256 = await sha256(file);
    const idempotencyKey = `sales-init-${crypto.randomUUID()}`;
    ...
    if (!upload.ok) throw new Error('SALES_STORAGE_UPLOAD_FAILED');
    ...
    await this.publish(initiated.batchId, token, {
      ...
      idempotencyKey: `sales-pub-${crypto.randomUUID()}`
    });
