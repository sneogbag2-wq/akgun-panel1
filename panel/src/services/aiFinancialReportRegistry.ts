...
export async function handleGetFinancialReconciliation(args: any): Promise<MetricResultEnvelope> {
  try {
    const res = await fetch(`${API_BASE_URL}/reports/advanced/reconciliation`);
    if (res.ok) {
...
export async function handleGetDataCoverage(args: any): Promise<MetricResultEnvelope> {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/reports/advanced/coverage?metricCode=${encodeURIComponent(metricCode)}`);
    if (res.ok) {
      const body = await res.json();
      if (body.data) {
        const cov = body.data;
